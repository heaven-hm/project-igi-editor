...
struct Gen {
    ...
    void emit32(uint32_t v) {
        for (int i = 0; i < 4; ++i) code.push_back((uint8_t)((v >> (8 * i)) & 0xFF));
    }
    void emit32At(uint32_t at, uint32_t v) {
        for (int i = 0; i < 4; ++i) code[at + i] = (uint8_t)((v >> (8 * i)) & 0xFF);
    }
    ...
    uint32_t internIdent(const std::string& name) {
        ...
        if (it != identIndex.end()) return it->second;
    ...
    void pushIdent(const std::string& name) {
        ...
        if (idx < 256) { emitOp(QVMOpType::PUSHIIB); emit8((uint8_t)idx); }
        else if (idx < 65536) { emitOp(QVMOpType::PUSHIIW); emit16((uint16_t)idx); }
        else { emitOp(QVMOpType::PUSHII); emit32(idx); }
    }
    void pushString(const std::string& s) {
        uint32_t idx = internString(s);
        if (idx < 256) { emitOp(QVMOpType::PUSHSIB); emit8((uint8_t)idx); }
        else if (idx < 65536) { emitOp(QVMOpType::PUSHSIW); emit16((uint16_t)idx); }
        else { emitOp(QVMOpType::PUSHSI); emit32(idx); }
    }
    void pushInt(int32_t v) {
        if (v == 0) { emitOp(QVMOpType::PUSH0); return; }
        if (v == 1) { emitOp(QVMOpType::PUSH1); return; }
        if (v == -1) { emitOp(QVMOpType::PUSHM); return; }
        // PUSHB / PUSHW use SIGNED ranges to match legacy GConv output.
        // E.g. 255 must use PUSHW (4 bytes) — PUSHB would re-interpret as -1.
        if (v >= -128 && v <= 127) { emitOp(QVMOpType::PUSHB); emit8((uint8_t)(int8_t)v); return; }
        if (v >= -32768 && v <= 32767) { emitOp(QVMOpType::PUSHW); emit16((uint16_t)(int16_t)v); return; }
        emitOp(QVMOpType::PUSH); emit32((uint32_t)v);
    }
    ...
    static QVMOpType BinaryOp(const std::string& op) {
        if (op == "+")  return QVMOpType::ADD;
        if (op == "-")  return QVMOpType::SUB;
        if (op == "*")  return QVMOpType::MUL;
        if (op == "/")  return QVMOpType::DIV;
        if (op == "<<") return QVMOpType::SHL;
        if (op == ">>") return QVMOpType::SHR;
        if (op == "&")  return QVMOpType::AND;
        if (op == "|")  return QVMOpType::OR;
        if (op == "^")  return QVMOpType::XOR;
        if (op == "&&") return QVMOpType::LAND;
        if (op == "||") return QVMOpType::LOR;
        if (op == "==") return QVMOpType::EQ;
        if (op == "!=") return QVMOpType::NE;
        if (op == "<")  return QVMOpType::LT;
        if (op == "<=") return QVMOpType::LE;
        if (op == ">")  return QVMOpType::GT;
        if (op == ">=") return QVMOpType::GE;
        if (op == "=")  return QVMOpType::ASSIGN;
        throw std::runtime_error("unknown binary operator: " + op);
    }
    static QVMOpType UnaryOp(const std::string& op) {
        if (op == "-") return QVMOpType::MINUS;
        if (op == "+") return QVMOpType::PLUS;
        if (op == "~") return QVMOpType::INV;
        if (op == "!") return QVMOpType::NOT;
    ...
    void emitExpr(const Node& n) {
        switch (n.kind) {
            case NodeKind::IntLit:    pushInt(n.i_val); return;
            case NodeKind::FloatLit:  pushFloat(n.f_val); return;
            case NodeKind::BoolLit:
                ...
                pushIdent(n.b_val ? "TRUE" : "FALSE");
                return;
            case NodeKind::StringLit: pushString(n.s_val); return;
            case NodeKind::IdentLit:  pushIdent(n.s_val); return;

            case NodeKind::Unary:
                emitExpr(*n.children[0]);
                emitOp(UnaryOp(n.s_val));
                return;

            case NodeKind::Binary:
                emitExpr(*n.children[0]);
                emitExpr(*n.children[1]);
                emitOp(BinaryOp(n.s_val));
                return;
    ...
    void emitCall(const Node& n) {
        ...
        for (size_t i = 0; i < n.children.size(); ++i) emit32(0);
    ...
    void emitStmt(const Node& n) {
        switch (n.kind) {
            case NodeKind::Block:
                for (const auto& c : n.children) emitStmt(*c);
                return;
...
std::vector<uint8_t> assemble(const Gen& g) {
    ...
    // Header
    out.push_back('L'); out.push_back('O'); out.push_back('O'); out.push_back('P');
    appendU32(out, 8);  // ver_major
    appendU32(out, 5);  // ver_minor
    appendU32(out, of_itable); appendU32(out, of_ivalue);
    appendU32(out, sz_itable); appendU32(out, sz_ivalue);
    appendU32(out, of_stable); appendU32(out, of_svalue);
    appendU32(out, sz_stable); appendU32(out, sz_svalue);
    appendU32(out, of_ctable); appendU32(out, sz_ctable);
    appendU32(out, 0);          // unknown_1
    appendU32(out, 0);          // unknown_2

    // ITable
    for (uint32_t v : itable) appendU32(out, v);
    // IValue
    out.insert(out.end(), ivalue.begin(), ivalue.end());
    // STable
    for (uint32_t v : stable) appendU32(out, v);
    // SValue
...
bool CompileToFile(const Node& program, const std::string& outPath, std::string* error) {
    CompileResult r = Compile(program);
    if (!r.ok) { if (error) *error = r.error; return false; }
    std::ofstream f(outPath, std::ios::binary | std::ios::trunc);
    if (!f) {
        if (error) *error = "failed to open output: " + outPath;
        return false;
    }
    f.write((const char*)r.binary.data(), (std::streamsize)r.binary.size());
    return (bool)f;
}
